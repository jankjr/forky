...
const run = async () => {
    let simulator = await (0, index_cjs_1.createSimulator)(mainProvider, "Reth");
    ...
    console.log("Alice balance:", await fork.getBalance(alice));
    console.log("Bob balance:", await fork.getBalance(bob));
    console.log("Execution result:", executionResult.receipt.status === 1 // true if the transaction was successful
    );
